...
class Region {
    ...
    exportToFile(filename) {
        ...
        file.write("lat,long,density\n");
        for (var i = 0; i < this._data.length; i++) {
            var e = this._data[i] || 0;
            var point = this.getIndexPoint(i);
            file.write(point.x / Region.LAT_LONG_PRECISION + "," + point.y / Region.LAT_LONG_PRECISION + "," + e + "\n");
        }
        file.end();
    }
    ...
    exportCellsToFile(filename) {
        ...
        file.write("x,y,lat,long,density\n");
        for (var i = 0; i < this._data.length; i++) {
            var e = this._data[i] || 0;
            var point = this.getIndexPoint(i);
            var x = Math.floor(i % size.x);
            var y = Math.floor(i / size.x);
            file.write(x + "," + y + "," + point.x / Region.LAT_LONG_PRECISION + "," + point.y / Region.LAT_LONG_PRECISION + "," + e + "\n");
        }
        file.end();
    }
