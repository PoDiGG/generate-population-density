...
class Region {
    ...
    addCode(point, code) {
        if (point.x < this.min.x || point.x > this.max.x || point.y < this.min.y || point.y > this.max.y) {
            throw new Error("Point " + JSON.stringify(point) + " was outside of bounds " + JSON.stringify(this.min) + " " + JSON.stringify(this.max));
        }
    ...
    exportEdgesToFile(filename, edges) {
        var file = fs.createWriteStream(filename);
        file.write("x1,y1,x2,y2,tripid,arrivaltime,departuretime\n");
        var size = new Point(this.max.x - this.min.x, this.max.y - this.min.y);
        for (var i = 0; i < edges.length; i++) {
            var e = edges[i];
            var index1 = this.getPointIndex(e.from);
            var index2 = this.getPointIndex(e.to);
            var x1 = Math.floor(index1 % size.x);
            var y1 = Math.floor(index1 / size.x);
            var x2 = Math.floor(index2 % size.x);
            var y2 = Math.floor(index2 / size.x);
            file.write(x1 + "," + y1 + "," + x2 + "," + y2 + "," + e.tripId + "," + e.arrivalTime + "," + e.departureTime + "\n");
        }
        file.end();
    }
